...
async.parallel([
    function (cb) {
        ...
        s.on('finish', function () {
            ...
            async.parallel([
                function (_cb) {
                    fs.writeFile(dataDir + '/entry.json', JSON.stringify(entries), _cb);
                },
                function (_cb) {
                    fs.writeFile(staticDir + '/entry.js', 'var __entries=' + JSON.stringify(entries) + ';', _cb);
                }
            ], cb);
        });
        s.on('error', cb);
    ...
    function (cb) {
        ...
        s.on('finish', function () {
            ...
            async.parallel([
                function (_cb) {
                    fs.writeFile(dataDir + '/master.json', JSON.stringify(masters), _cb);
                },
                function (_cb) {
                    fs.writeFile(staticDir + '/master.js', 'var __masters=' + JSON.stringify(masters) + ';', _cb);
                }
            ], cb);
        });
        s.on('error', cb);
